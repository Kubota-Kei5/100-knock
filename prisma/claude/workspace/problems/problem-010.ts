...
async function transactionProcessing() {
  try {
    ...
    const [user1, user2] = await /* ここを実装してください */
    ...
    const blogResult = await prisma.$transaction(async (prisma) => {
      ...
      const newPost = await /* ここを実装してください */
      
      // TODO: 田中太郎の投稿数をカウント
      const postCount = await /* ここを実装してください */
      
      // TODO: ユーザーのage フィールドに投稿数を保存（統計として）
      const updatedUser = await /* ここを実装してください */
    ...
    try {
      ...
      const tempUser = await /* ここを実装してください */
    ...
    const deleteResult = await prisma.$transaction(async (prisma) => {
      // まず対象ユーザーを検索
      const targetUser = await /* 山田花子を検索 */
      ...
      if (targetUser) {
        ...
        // TODO: 全体の投稿数を再カウント
        const totalPosts = await /* ここを実装してください */
    ...
    const [userCount, postCount] = await /* ここを実装してください */
