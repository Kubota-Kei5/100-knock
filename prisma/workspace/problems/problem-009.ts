...
async function dangerousDeleteOperations() {
  try {
    ...
    console.log(`✅ 田中太郎の投稿を${deletedTaroPosts.count}件削除しました`);
    ...
    console.log("\n📋 タスク3: CASCADE削除の危険性");
    console.log(
      "⚠️ もしCASCADE設定があれば、ユーザー削除で投稿も自動削除されます"
    );
    ...
    try {
      ...
      if (jiro) {
        ...
        // 削除前に投稿数を確認
        const jiroPosts = await prisma.post.findMany({
          where: {
            authorId: jiro.id,
          },
          _count: true,
        ...
        if (jiroPosts.length > 0) {
          console.log(
            `   → CASCADE設定により投稿${jiroPosts.length}件も自動削除されました`
          );
        }
      }
    } catch (error) {
    ...
    if (targetCount > 0) {
      ...
      const bulkDeleted = await prisma.user.deleteMany({
        where: {
          age: null,
        },
      });
    ...
    console.log(
      `残りユーザー数: ${finalUserCount}人、投稿数: ${finalPostCount}件`
    );
